...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String userNameAttributeName = userRequest.getClientRegistration() //로그인 진행시 키가 되는 필드값(pk), 구글은 기본지원(sub) 네이버카카오는 지원안함
                .getProviderDetails().getUserInfoEndpoint()
                .getUserNameAttributeName();
        OAuthAttributes attributes = OAuthAttributes.of(registrationId, userNameAttributeName, oAuth2User.getAttributes());

        User user = saveOrUpdate(attributes);
        httpSession.setAttribute("user", new SessionUser(user)); //session에 사용자 정보 저장하는 Dto, 새로 만들어서 사용
    ...
    private User saveOrUpdate(OAuthAttributes attributes){ //구글 정보 업데이트시 사용
